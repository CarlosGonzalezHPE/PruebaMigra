...
sub replace_vars {
  ...
  while($string =~ /<%(([\w\-]+\.)*)(@?\w+(\[\w+\])*)%>/){
    $replate_level++;
    if($replate_level>1000){
      logError("Recursive replace detected afeter ".$replate_level." loops. Replace string is: '".$string."'");
      die "Recursive replace detected afeter ".$replate_level." loops. Replace string is: '".$string."'";
    }
    ...
    $ok=0;
    if($original_variable eq "FECHA_BUILD") {
      (my $sec, my $min, my $hour, my $mday, my $mon, my $year, my $wday, my $yday, my $isdst) = localtime(time);
      my $build_date = sprintf("%02d/%02d/%04d", $mday, 1 + $mon, 1900 + $year);
      $_=$string;
      s/<%$original_variable%>/$build_date/g;
      $string=$_;
      $ok=1;
    }
    else {
      ...
      while (defined ($input_line=$INPUT[$lineno])) {
        ...
        if($input_line =~ /^[ \t]*\[\#(.*)\#\][ \t]*$/){ # [#<COMMAND>#]
          if($1 =~ /^SECTION_BEGIN:(\w+)$/){
            ...
            if($section_enabled{$level} && exists $replaces{$section.'.@ITERATION'}){

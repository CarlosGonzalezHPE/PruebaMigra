...
function process
{
  ...
  then
    logInfo "Directory '/var/opt/SIU_MANAGER' is not accessible. Autoprovisioning file generation does not apply"
    return 0
  fi
  ...
  then
    ...
    then
      ...
      then
        ...
        /opt/scripts/distribution/sendFiles/sh/sendFiles.sh -o DSI_VOLTE ${OUTPUT_DIR}/LOG_VOLTE/$(basename ${VOLTE_FILEPATH})
        if [ $? -ne 0 ]
        then
...
SCRIPT_BASEDIR=/opt/scripts/provisioning/generateAutoprovisioningFiles
